...
import { getTierLabelKey } from "../helpers/company-table-helpers";
...
export function ChangeTierDialog({ company, onConfirm, onClose }: ChangeTierDialogProps) {
  ...
  const handleConfirm = () => {
    ...
    if (result.success) {
      ...
      setError(result.error || t("companies:dialog.change_tier_error"));
    }
  };
          ...
          <>
            <DialogHeader>
              <DialogTitle>
                ...
                {t("companies:dialog.change_tier_title")}
              </DialogTitle>
            ...
            <DialogBody>
              <div className="space-y-2">
                <label className="text-xs font-semibold text-muted-foreground">
                  {t("companies:dialog.select_tier_label")}
                </label>
                <Select
                  value={selectedTier}
              ...
              <div className="p-3 rounded-lg border border-border bg-secondary/20 text-xs space-y-1">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("companies:table.projects")}</span>
                  <span className="font-semibold text-foreground">
                    {Number.isFinite(limits.maxProjects) ? limits.maxProjects : t("companies:limit_unlimited")}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("companies:table.personnel")}</span>
                  <span className="font-semibold text-foreground">
                    {Number.isFinite(limits.maxPersonnel) ? limits.maxPersonnel : t("companies:limit_unlimited")}
                  </span>
                </div>
              </div>
