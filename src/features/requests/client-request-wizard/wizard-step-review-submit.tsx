      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <Card className="border-border bg-card shadow-sm rounded-xl">
            ...
            <CardContent className="space-y-3 text-xs">
              <div className="grid grid-cols-1 gap-1.5">
                <span className="text-muted-foreground block">{t("requests:wizard.steps.type")}</span>
                <span className="font-semibold text-foreground text-sm">{getRequestTypeLabel(values.requestType)}</span>
              </div>
              <div className="pt-2 border-t border-border/50 space-y-2">
                {values.requestType === "new_license" && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.landPlotNumber")}:</span>
                      <span className="font-semibold text-foreground">{values.landPlotNumber}</span>
                    </div>
                    {values.gpsCoordinates && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.gpsCoordinates")}:</span>
                        <span className="font-semibold text-foreground">{values.gpsCoordinates}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.buildingStatus")}:</span>
                      <span className="font-semibold text-foreground uppercase">
                        {values.buildingStatus === "ready"
                          ? t("requests:wizard.serviceDetails.buildingStatusReady")
                          : values.buildingStatus === "under_construction"
                          ? t("requests:wizard.serviceDetails.buildingStatusUnderConstruction")
                          : values.buildingStatus === "renovation"
                          ? t("requests:wizard.serviceDetails.buildingStatusRenovation")
                          : values.buildingStatus}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.licensePurpose")}:</span>
                      <span className="font-semibold text-foreground">{values.licensePurpose}</span>
                    </div>
                    {values.currentSafetyEquipment && (
                      <div className="pt-1.5">
                        <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.currentSafetyEquipment")}:</span>
                        <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.currentSafetyEquipment}</span>
                      </div>
                    )}
                  </>
                )}

                {values.requestType === "maintenance_contract" && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.preferredVisitDate")}:</span>
                      <span className="font-semibold text-foreground">{values.preferredVisitDate}</span>
                    </div>
                    {values.lastMaintenanceDate && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.lastMaintenanceDate")}:</span>
                        <span className="font-semibold text-foreground">{values.lastMaintenanceDate}</span>
                      </div>
                    )}
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.onSiteCoordinatorName")}:</span>
                      <span className="font-semibold text-foreground">{values.onSiteCoordinatorName}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.onSiteCoordinatorPhone")}:</span>
                      <span className="font-semibold text-foreground">{values.onSiteCoordinatorPhone}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.oldContractAvailable")}:</span>
                      <span className="font-semibold text-foreground">
                        {values.oldContractAvailable 
                          ? t("requests:wizard.serviceDetails.yes") 
                          : t("requests:wizard.serviceDetails.no")}
                      </span>
                    </div>
                    <div className="pt-1.5">
                      <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.existingSafetySystems")}:</span>
                      <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.existingSafetySystems}</span>
                    </div>
                  </>
                )}

                {values.requestType === "engineering_blueprint" && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.buildingFloors")}:</span>
                      <span className="font-semibold text-foreground">{values.buildingFloors}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.constructionStatus")}:</span>
                      <span className="font-semibold text-foreground uppercase">
                        {values.constructionStatus === "not_started"
                          ? t("requests:wizard.serviceDetails.constructionStatusNotStarted")
                          : values.constructionStatus === "foundation"
                          ? t("requests:wizard.serviceDetails.constructionStatusFoundation")
                          : values.constructionStatus === "finishing"
                          ? t("requests:wizard.serviceDetails.constructionStatusFinishing")
                          : values.constructionStatus}
                      </span>
                    </div>
                    <div className="pt-1.5">
                      <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.blueprintScope")}:</span>
                      <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.blueprintScope}</span>
                    </div>
                    {values.requiredSystems && (
                      <div className="pt-1.5">
                        <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.requiredSystems")}:</span>
                        <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.requiredSystems}</span>
                      </div>
                    )}
                    {values.engineeringNotes && (
                      <div className="pt-1.5">
                        <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.engineeringNotes")}:</span>
                        <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.engineeringNotes}</span>
                      </div>
                    )}
                  </>
                )}

                {values.requestType === "technical_report" && (
                  <>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.reportType")}:</span>
                      <span className="font-semibold text-foreground uppercase">
                        {values.reportType === "instant"
                          ? t("requests:wizard.serviceDetails.reportTypeInstant")
                          : values.reportType === "non_instant"
                          ? t("requests:wizard.serviceDetails.reportTypeNonInstant")
                          : values.reportType === "compliance"
                          ? t("requests:wizard.serviceDetails.reportTypeCompliance")
                          : values.reportType}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{t("requests:wizard.serviceDetails.inspectionNeeded")}:</span>
                      <span className="font-semibold text-foreground">
                        {values.inspectionNeeded 
                          ? t("requests:wizard.serviceDetails.yes") 
                          : t("requests:wizard.serviceDetails.no")}
                      </span>
                    </div>
                    <div className="pt-1.5">
                      <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.caseDescription")}:</span>
                      <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.caseDescription}</span>
                    </div>
                    {values.buildingLicenseContext && (
                      <div className="pt-1.5">
                        <span className="text-muted-foreground block mb-0.5">{t("requests:wizard.serviceDetails.buildingLicenseContext")}:</span>
                        <span className="text-foreground block bg-secondary/30 p-2 rounded">{values.buildingLicenseContext}</span>
                      </div>
                    )}
                  </>
                )}
